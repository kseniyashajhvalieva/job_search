...
class DBManager:
    ...
    def get_companies_and_vacancies_count(self) -> list[tuple[int, str, int]]:
        ...
        cur = conn.cursor()
        cur.execute(
            "SELECT employers.employer_id, name_emp, COUNT(DISTINCT name_vac) FROM employers INNER JOIN vacancies "
    ...
    def get_all_vacancies(self) -> list[tuple[int, str, str, str, str, str]]:
        """Получает список всех вакансий с указанием названия компании,
        названия вакансии и зарплаты и ссылки на вакансию."""
        conn = get_connection(self.dbname)
        cur = conn.cursor()
        cur.execute(
            "SELECT employers.employer_id, name_emp, name_vac, salary_from|| ' - ' ||salary_to as salary, "
    ...
    def get_vacancies_with_higher_salary(self) -> list[tuple[str, int]]:
        ...
        cur = conn.cursor()
        cur.execute(
            "SELECT name_vac, salary_from FROM vacancies "
